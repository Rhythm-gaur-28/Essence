...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      toast.error('Invalid credentials. Try: admin@brandessence.com or priya@email.com');
    }
  };
    ...
    <div className="min-h-[70vh] flex items-center justify-center px-4">
      <div className="bg-card rounded-xl p-8 border border-border w-full max-w-md shadow-sm animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="btn-gold w-full text-sm">Sign In</button>
        </form>
        <p className="text-center text-sm text-muted-foreground mt-4">
          New here? <Link to="/register" className="text-primary hover:underline font-medium">Register</Link>
        </p>
